...
function loadPage(a){var b=new XMLHttpRequest;b.onreadystatechange=function(){if(4==this.readyState&&200==this.status){activePage=a;resetGameplayCtrls();saveGameDate();document.getElementById("gameplay-container").innerHTML=b.responseText;var d="init"+a.replace("/","").replace(".","").replace("pages","").replace("html","");if(window[d]&&"function"==typeof window[d])window[d]();else console.log("no init fn for: "+d)}};b.onloadstart=function(){};b.onloadend=function(){};b.onerror=function(){};b.onabort=
function(){};b.open("GET",a,!0);b.setRequestHeader("Cache-Control","no-cache, no-store, max-age=0");b.setRequestHeader("Expires","Tue, 01 Jan 1980 1:00:00 GMT");b.setRequestHeader("Pragma","no-cache");try{b.send()}catch(d){console.error(d.message)}}var isRendering=!1;
...
"You should NEVER trust a ghoul's advice.",goDie)})}function drinkPotion(a,b){showMsg("info",b?"You drink the potion, and feel invigorated!<br><br>2 health points have been added to your total":"You drink the potion, and feel awful!<br><br>2 health points have been removed from your total",function(){hp=b?Number(hp)+Number(2):Number(hp)-Number(2);loadPage(a)},null)}
